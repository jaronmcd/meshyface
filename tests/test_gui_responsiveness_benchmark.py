...
def test_dashboard_js_includes_gui_responsiveness_benchmark(
    ...
    assert_tokens_present(
        ...
        (
            ...
            "meshGuiBenchmarkOpenButtonMenu",
            "meshGuiBenchmarkOpenLayoutMenu",
            '"interact:layout-menu-open"',
            '"interact:favorite-menu-open"',
...
def test_gui_benchmark_markdown_report_summarizes_key_metrics() -> None:
    ...
    assert "# Meshyface GUI smoke test" in markdown
...
def test_gui_smoke_comment_reports_health_without_performance_comparison() -> None:
    report = _load_report_module()
    result = {"ok": False, "summary": {"samples": 25, "errors": 2, "longTaskCount": 1}}
    comment = report.render_pr_comment(result, run_url="https://example.test/run")
    assert "<!-- meshyface-gui-benchmark-report -->" in comment
    assert "## GUI smoke test" in comment
    assert "| Result | fail |" in comment
    assert "| Errors | 2 |" in comment
    assert "does not measure performance improvements" in comment
    assert "Before" not in comment
    assert "Delta" not in comment
