...
- Added targeted tests:
  ...
  - tighter node-history / online-activity / saved-count / capability wrapper delegation assertions in `tests/test_history_store_io_wrappers.py`
...
- `tracker_runtime_receive_bindings._resolve_tracker_node_id_from_num(...)` now fails safe (`None`) on resolver exceptions so transient node-id lookup faults do not crash receive-path packet handling.
...
- Tightened top-level runtime callable contracts in `meshdash/runtime_types.py` by replacing broad aliases for HTML render/HTTP handler and state/history/chat endpoint callables (`RenderHtmlFn`, `MakeHttpHandlerFn`, `StateFn`, `NodeHistoryFn`, `OnlineActivityFn`, `SendChatFn`) with explicit protocol signatures used by runtime loader/server wiring.
- Tightened remaining runtime wiring aliases in `meshdash/runtime_types.py` by replacing broad `Build*`/send/local-chat callable aliases with explicit protocol signatures (`BuildStateWithSensitiveFn`, `BuildStateFn`, `BuildStateSnapshotLoaderFn`, `BuildNodeHistoryLoaderFn`, `BuildOnlineActivityLoaderFn`, `BuildSendChatLoaderFn`, `SendChatMessageFn`, `RecordLocalChatFn`, `RawGetLocalNodeIdFn`) and split raw vs wrapped reaction-send contracts (`SendReactionPacketWithModulesFn` vs `SendReactionPacketFn`) through `wiring_adapters.py` / `wiring_runtime.py`.
- Added explicit dashboard setup/loader wiring protocols and applied them in runtime context assembly to remove remaining broad runtime-context callables:
...
- Routed state assembly through typed-first collaborator paths while preserving compatibility fallbacks:
  - `state_service.py` now defaults to typed node collection (`collect_nodes_typed(...)`) for internal state assembly.
  - `state.py` now wires `build_state(...)` through `collect_nodes_typed(...)` while preserving `collect_nodes(...)` dict facade compatibility.
  - `state_tracker.py` now prefers `tracker.snapshot_typed(...)` when available before falling back to `snapshot(...)` + coercion.
...
- `tracker_runtime_receive.record_tracker_receive_unlocked(...)` now expires pending deliveries in a `finally` block so receive-path expiration is guaranteed even when dependency assembly or injected packet-record callbacks raise, while preserving the original processing exception if expiration also fails.
...
- Tightened state-service revision boundary:
  - `build_dashboard_state_typed(...)`, summary contracts, and summary fallback now require typed `RevisionInfo`
  - `build_dashboard_state(...)` retains mapping compatibility via `coerce_revision_info(...)` at the API wrapper boundary.
