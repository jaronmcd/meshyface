...
- Tightened tracker runtime port-counter contracts from generic `object` to `MutableMapping[str, int]` across receive/record dependency and callback signatures (`runtime_types.py`, `tracker_runtime_packet_contracts.py`, `tracker_runtime_record.py`, `tracker_runtime_record_dependencies.py`) to match actual mutation behavior in parsed-packet processing.
