...
its nearest city, and the ticker continuously displays the top cities. Packets
without a resolvable city are counted as **No city**, and **Seen** shows the
latest packet time. Scores reset when the script worker restarts, so the example
works without configuration in any region.
